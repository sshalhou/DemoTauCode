...
def muTauClassification_forQCD(chain, index):
    ...
    if ('btag_low' in str(localClassIf) or 'btag_high' in str(localClassIf)) and 'nobtag' not in str(localClassIf) :
...
def classifyZDecay_FiXDONETUSEnal(chain,maxPairTypeAndIndex):
    classification = 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
    i=maxPairTypeAndIndex[0]
    ########################
    # check reco-gen matches
