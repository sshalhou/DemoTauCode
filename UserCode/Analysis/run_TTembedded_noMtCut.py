...
# loop over eTau pairs and store all that pass in list
			...
			for index in range(0, chain.eT_correctedSVFitMass.size()):
				passesCuts = True
				if passesEmbeddedTTbarSelectionETau_NoMtCut(chain,index,UseNewTriggers,Verbose) is False:
					passesCuts = False
...
# loop over muTau pairs and store all that pass in list
			...
			for index in range(0, chain.muT_correctedSVFitMass.size()):
				passesCuts = True
				if passesEmbeddedTTbarSelectionMuTau_NoMtCut(chain,index,UseNewTriggers,Verbose) is False:
					passesCuts = False
				if passesCuts is True:
