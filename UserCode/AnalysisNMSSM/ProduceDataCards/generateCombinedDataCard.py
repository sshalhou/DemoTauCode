...
for chan in range(0,len(CHANNELS)):
	...
	for direc in range(0,len(DIRLIST)):
		...
		for hist in range(0, len(HISTLIST)):
			#if RejectHistogram(DIRNAME, HISTLIST[hist]) is True:
			#	print "===> reject ====> ", DIRNAME+"/"+HISTLIST[hist]
	...
	BTAG_TOTALSDICT['ZTT_lowMass'] = 0.0
	...
	print 'Z->TT (low mass)',  BTAG_TOTALSDICT['ZTT_lowMass']
	...
	print 'data/MC', BTAG_TOTALSDICT['data_obs']/(BTAG_TOTALSDICT['QCD']+BTAG_TOTALSDICT['W']+
							...
							BTAG_TOTALSDICT['TT']+
							BTAG_TOTALSDICT['VV']+BTAG_TOTALSDICT['ZTT_lowMass'])
