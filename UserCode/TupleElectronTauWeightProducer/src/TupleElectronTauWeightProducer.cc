...
TupleElectronTauWeightProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (std::size_t i = 0; i < electronTaus->size(); ++i)
  {
    ...
    CurrentElectronTauWeight.set_hepNUP(hepNUP);
    ...
    CurrentElectronTauWeight.set_upPOWHEGscale(upPOWHEGscale);
    CurrentElectronTauWeight.set_downPOWHEGscale(downPOWHEGscale);
